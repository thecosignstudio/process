...
產出：受測者應對你分享的設計留下評論（例如：透過 InVision）
...
由品保專家主持設計缺陷掃除。在一開始，告知其他人專案內容，接著描述典型使用者的細節。解釋需要參與者完成的事情，但是不要預先解釋任何設計理念。每個人都需參與產品設計師提供的原型設計與任務。
產出：受測者感受到的任何錯誤、懷疑將被他們的邀請者記錄，例如：透過 InVision 的評論區。
