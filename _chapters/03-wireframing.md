...
{% include dosndonts.html introduction="1. 色彩可能分散注意力，確保你的設計專注於你正在創造的體驗並非以視覺設計取向為解決方案。
" dos="
以低飽和度的色調創作線框圖。
| 即便只有在有助於定義設計背後的理念時才使用顏色；也要盡可能地限制自己使用極少的色彩。
（即便只有使用色彩才能助於定義設計的理念）" donts=" 除非有確切的用途，否則不要在線框設計中使用色彩。
...
{% include dosndonts.html introduction="3. 使用真實的內容：寫作是一種設計技能，可以更好地幫助你理解客戶及故事。" dos="
如果沒有真實的內容：根據自己研究的競品內容。
...
{% include dosndonts.html introduction="5. 設定明確的期望：確保客戶理解過程背後的原則。
" dos="
在開始設計之前，請告知客戶期望的結果。|
儘早地、經常地、清晰地溝通。|
在第一次展示您的線框時，與客戶進行即時（實時）溝通/會議/通話是個好主意——這樣你就有機會回答所有問題並避免誤解。
" donts="
不要假設你的客戶知道如何使用協作工具或知道線框稿不是最終設計——總是教育。|
避免留下沒有交代的複雜設計過程。"%}

{% include dosndonts.html introduction="
6. 使用正確的工具完成工作。在 Netguru, 我們一定會在 Sketch 製作線框稿（它彈性、易用、無需在其他app中重新創建佈局），但請記住，不要受到一個工具的限制。
" dos="
使用任何一個你看到合適的工具；舉例來說，素描與白板。
" donts="
別讓軟體成為障礙。"%}

{% include dosndonts.html introduction="7. 不是為了自己設計。（你不是你的設計）(你的設計不是你的設計)" dos="
準備好經常轉換並擁抱快速構思。
" donts="
別太執著於你的線框稿，但與此同時，也要記住捍衛值得捍衛的想法。"%}

{% include dosndonts.html introduction="8. 保持簡單及選擇性。" dos="
一開始就選擇最好的想法，避免那些肯定無法執行的。
" donts="
在思考設計最後的階段，嘗試只創建必要的部份，不要過度使用線框稿。"%}

{% include dosndonts.html introduction="9. 掌控對話。" dos="
透過掌控對話來支持線框稿：如果離題，請將對話拉回你想要的。
" donts="
不要害怕禮貌性地打斷並提出有意義的開放性問題。"%}

{% include dosndonts.html introduction="10. 考量到最終設計的線框稿。" dos="
如果在創建線框稿時有考量到最終設計：可重複使用的符號，精確的元素，合適的字體，你將可省去大量的工作。 |
與開發人員討論有效率的技術解決方案。
" donts="
